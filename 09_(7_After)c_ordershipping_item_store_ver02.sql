...
;
/* 約10分かかっている */
INSERT INTO gu_free.c_ordershipping_item_store_new_061202 WITH w_shipping AS(
    SELECT
        business_datetime
        ,purchase_order_id
        ,purchase_member_id
        ...
        /* 20190603 START 修正・追加ロジック plu単位のためunit_retailは加重平均ロジック不要 */
    ...
    WHERE
        ...
        /* 直近10日分 */
        purchase_ec_flag IS FALSE
    AND pk_purchase_tran_type IN('SALE')
    AND DATEADD('day', - 10, CAST(CONVERT_TIMEZONE('JST', GETDATE()) AS date)) <= TRUNC(business_datetime)
    GROUP BY
        business_datetime
        ,purchase_order_id
        ,purchase_member_id
