...
;
--/* c_ordershipping_item_all(return)
...
WHERE
    ...
    trading_type = 'return'
;
INSERT INTO gu_free.c_ordershipping_item_all_new_061201 WITH item_base_info as(
    SELECT
        pk_order_id
        ,pk_tran_type
        ,pk_item_cd
        ...
        ,SUM(juchu_f_base_cost_amt_lcl * juchu_quantity) / SUM(juchu_quantity) AS juchu_unit_cost_sku
        ...
        ,address_zipcode
        ,address_addr_pref
        ,MAX(item_pk_date) AS item_pk_date
...
--         )
--        SUBSTRING(pk_juchu_order_id, 3, 6) between '190404' and '190406'
--         AND
-- 		(
--             purchase_type = 'ITEM'
--         OR  purchase_type IS NULL
...
--         ,pk_tran_type
--         ,item_pk_date
--item_pk_dateはpurchase_order_datetimeをTRUNCして作成しているため、
--f_itemのpk_dateのMAXの絞込みはできない
--        f_item
--)
...
*/
    LEFT JOIN
        gu_free.f_item item
    ON  a.item_max_plu = item.pk_catalog_l3_product_id
    AND a.item_pk_date = item.pk_date
;
COMMIT
;